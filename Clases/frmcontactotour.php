...
class frmContactoTour
{
	...
	{
		$conexion = new dbc();
		$result = $conexion->prepare("UPDATE ".$this->tabla." SET nombre = ?, tel = ?, email = ?, comentarios = ? WHERE idFrmContactoTour = ?");
	...
	{
		$conexion = new dbc();
		$result = $conexion->prepare("SELECT idFrmContactoTour,nombre,tel,email,comentarios FROM ".$this->tabla." WHERE idFrmContactoTour=?");
		$result->bind_param('i', $this->id);
		$result->execute();
	...
	{
		$conexion = new dbc();
		$result = $conexion->query("SELECT idFrmContactoTour,nombre,tel,email,comentarios FROM ".$this->tabla);
		$resultados =array();
